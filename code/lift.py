''' (4)
obtain multiple lift values at once, 
edit the edmunds_pair_keys.txt file by putting the entities and attributes (separated by commas) for which you want lift values.  Save it as a text file, and put in the Python27 directory. Now run the Liftcalculations.py script which will take the edmunds_pair_keys.txt file as well as Edmunds_Extraction.csv to calculate all possible lift values. The output is shown on the screen and also saved as:Lift_Values.csv.  
'''
import csv
