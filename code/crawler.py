'''(1)
This script crawls and fetches content from Edmunds.com discussion forums. 
You need to change the number of pages to fetch (e.g., 250), and the starting URL -- currently set to the Entry Level Luxury Forum, which is what you want for the assignment.
The output file name needs to be changed to make sure that if you run the crawler multiple times, a previously generated output CSV file does not get overwritten. 
Output: edmunds_extraction.csv
'''
...
while urlnumber<400: # Give the page number to end with
